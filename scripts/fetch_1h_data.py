...
r"""
================================================================================
1-HOUR PRICE SNAPSHOT INGEST SCRIPT
================================================================================
What:
    - Polls the OSRS Wiki 1-hour price snapshot endpoint and inserts a single
      OneHourTimeSeries row per item for each snapshot timestamp.
Why:
...
from Website.models import OneHourTimeSeries  # noqa: E402
...
# How: Passed as batch_size to OneHourTimeSeries.objects.bulk_create.
...
def load_item_mapping():
    ...
    Why: We need item names for denormalized storage in OneHourTimeSeries.
...
def snapshot_already_ingested(snapshot_timestamp):
    ...
    """
    return OneHourTimeSeries.objects.filter(timestamp=snapshot_timestamp).exists()
...
def build_timeseries_objects(snapshot_data, lookup, snapshot_timestamp):
    """
    Build OneHourTimeSeries objects from snapshot data.
    ...
    Returns:
        list: List of OneHourTimeSeries objects ready for bulk_create.
    """
    ...
    for item_id_raw, item_payload in snapshot_data.items():
        ...
        objects_to_insert.append(OneHourTimeSeries(
            item_id=item_id,
            item_name=item_name,
...
def fetch_and_store_snapshot(lookup):
    ...
    # Why: OneHourTimeSeries.timestamp is a CharField, and we need stable comparison.
    ...
    with transaction.atomic():
        for batch in chunk_list(objects_to_insert, BULK_INSERT_BATCH_SIZE):
            ...
            OneHourTimeSeries.objects.bulk_create(batch, batch_size=BULK_INSERT_BATCH_SIZE)
            inserted_count += len(batch)

    log(f"Inserted {inserted_count} OneHourTimeSeries rows.")
    return inserted_count


def main():
    """
    Main execution loop for the 1-hour snapshot ingestor.
