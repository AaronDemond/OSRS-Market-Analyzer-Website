...
r"""
================================================================================
5-MINUTE PRICE SNAPSHOT INGEST SCRIPT
================================================================================
What:
    - Polls the OSRS Wiki 5-minute price snapshot endpoint and inserts a single
      FiveMinTimeSeries row per item for each snapshot timestamp.
Why:
...
How:
    ...
    - Fetch the /5m snapshot (single API call).
    ...
    - Build FiveMinTimeSeries objects, leaving missing fields blank (None or 0 when
      the model does not permit NULL), and bulk-insert in a transaction.
    - Loop forever: fetch immediately, then sleep for 5 minutes between cycles.
================================================================================
"""
...
from Website.models import FiveMinTimeSeries  # noqa: E402
...
# What: Root URL used for the 5m snapshot endpoint.
# Why: Centralized for maintainability and reuse.
# How: Combine with "/5m" to build the request URL.
...
# REQUEST_TIMEOUT_SECONDS: HTTP timeout for the 5m snapshot request
...
# How: Passed as batch_size to FiveMinTimeSeries.objects.bulk_create.
BULK_INSERT_BATCH_SIZE = 500

# SLEEP_INTERVAL_SECONDS: Delay between snapshot polls
# What: Sleep duration between fetch cycles.
# Why: The endpoint updates every 5 minutes, so we poll on that cadence.
...
def load_item_mapping():
    ...
    Why: We need item names for denormalized storage in FiveMinTimeSeries.
...
def fetch_5m_snapshot():
    """
    Fetch the latest 5-minute price snapshot from the OSRS Wiki API.

    What: GET /5m and parse the JSON response.
    Why: The 5m endpoint returns all item price data and a snapshot timestamp.
    ...
    """
    try:
        # response: Raw HTTP response from the 5m snapshot endpoint.
        ...
        log(f"ERROR: 5m API request failed: {exc}")
        return None

    if response.status_code != 200:
        log(f"ERROR: 5m API returned status {response.status_code}")
        return None
    ...
    if not isinstance(payload, dict):
        log("ERROR: 5m API payload is not a JSON object.")
        return None

    if "data" not in payload or "timestamp" not in payload:
        log("ERROR: 5m API payload missing required keys (data/timestamp).")
        return None
...
def snapshot_already_ingested(snapshot_timestamp):
    ...
    What: Checks for any FiveMinTimeSeries row with the same timestamp.
    ...
    """
    return FiveMinTimeSeries.objects.filter(timestamp=snapshot_timestamp).exists()
...
def build_timeseries_objects(snapshot_data, lookup, snapshot_timestamp):
    """
    Build FiveMinTimeSeries objects from snapshot data.
    ...
    Returns:
        list: List of FiveMinTimeSeries objects ready for bulk_create.
    """
    ...
    for item_id_raw, item_payload in snapshot_data.items():
        ...
        objects_to_insert.append(FiveMinTimeSeries(
            item_id=item_id,
            item_name=item_name,
...
def fetch_and_store_snapshot(lookup):
    """
    Fetch a single 5m snapshot and insert it into the database (if new).
    ...
    """
    # snapshot: Raw JSON payload for the current 5m snapshot.
    snapshot = fetch_5m_snapshot()
    if snapshot is None:
        return 0

    # snapshot_timestamp: Canonical snapshot timestamp as a string.
    # Why: FiveMinTimeSeries.timestamp is a CharField, and we need stable comparison.
    snapshot_timestamp = str(snapshot.get("timestamp"))

    if snapshot_already_ingested(snapshot_timestamp):
        existing_rows = FiveMinTimeSeries.objects.filter(timestamp=snapshot_timestamp).count()
        log(
    ...
    log(
        ...
        f"FiveMinTimeSeries rows, inserted {inserted_count}, "
...
def main():
    """
    Main execution loop for the 5-minute snapshot ingestor.

    What: Load mapping, build lookup, then run a fetch/sleep loop forever.
    Why: Keeps the script running continuously on a 5-minute cadence.
