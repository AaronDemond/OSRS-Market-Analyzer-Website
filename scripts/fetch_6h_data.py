...
r"""
================================================================================
6-HOUR PRICE SNAPSHOT INGEST SCRIPT
================================================================================
What:
    - Polls the OSRS Wiki 6-hour price snapshot endpoint and inserts a single
      SixHourTimeSeries row per item for each snapshot timestamp.
Why:
...
How:
    ...
    - Fetch the /6h snapshot (single API call).
    ...
    - Build SixHourTimeSeries objects, leaving missing fields blank (None or 0 when
      the model does not permit NULL), and bulk-insert in a transaction.
    - Loop forever: fetch immediately, then sleep for 6 hours between cycles.
================================================================================
"""
...
from Website.models import SixHourTimeSeries  # noqa: E402
...
# What: Root URL used for the 6h snapshot endpoint.
# Why: Centralized for maintainability and reuse.
# How: Combine with "/6h" to build the request URL.
...
# REQUEST_TIMEOUT_SECONDS: HTTP timeout for the 6h snapshot request
...
# How: Passed as batch_size to SixHourTimeSeries.objects.bulk_create.
BULK_INSERT_BATCH_SIZE = 500

# SLEEP_INTERVAL_SECONDS: Delay between snapshot polls
# What: Sleep duration between fetch cycles.
# Why: The endpoint updates every 6 hours, so we poll on that cadence.
...
def load_item_mapping():
    ...
    Why: We need item names for denormalized storage in SixHourTimeSeries.
...
def fetch_6h_snapshot():
    """
    Fetch the latest 6-hour price snapshot from the OSRS Wiki API.

    What: GET /6h and parse the JSON response.
    Why: The 6h endpoint returns all item price data and a snapshot timestamp.
    ...
    """
    try:
        # response: Raw HTTP response from the 6h snapshot endpoint.
        ...
        log(f"ERROR: 6h API request failed: {exc}")
        return None

    if response.status_code != 200:
        log(f"ERROR: 6h API returned status {response.status_code}")
        return None
    ...
    if not isinstance(payload, dict):
        log("ERROR: 6h API payload is not a JSON object.")
        return None

    if "data" not in payload or "timestamp" not in payload:
        log("ERROR: 6h API payload missing required keys (data/timestamp).")
        return None
...
def snapshot_already_ingested(snapshot_timestamp):
    ...
    What: Checks for any SixHourTimeSeries row with the same timestamp.
    ...
    """
    return SixHourTimeSeries.objects.filter(timestamp=snapshot_timestamp).exists()
...
def build_timeseries_objects(snapshot_data, lookup, snapshot_timestamp):
    """
    Build SixHourTimeSeries objects from snapshot data.
    ...
    Returns:
        list: List of SixHourTimeSeries objects ready for bulk_create.
    """
    ...
    for item_id_raw, item_payload in snapshot_data.items():
        ...
        objects_to_insert.append(SixHourTimeSeries(
            item_id=item_id,
            item_name=item_name,
...
def fetch_and_store_snapshot(lookup):
    """
    Fetch a single 6h snapshot and insert it into the database (if new).
    ...
    """
    # snapshot: Raw JSON payload for the current 6h snapshot.
    snapshot = fetch_6h_snapshot()
    if snapshot is None:
        return 0

    # snapshot_timestamp: Canonical snapshot timestamp as a string.
    # Why: SixHourTimeSeries.timestamp is a CharField, and we need stable comparison.
    ...
    with transaction.atomic():
        for batch in chunk_list(objects_to_insert, BULK_INSERT_BATCH_SIZE):
            ...
            SixHourTimeSeries.objects.bulk_create(batch, batch_size=BULK_INSERT_BATCH_SIZE)
            inserted_count += len(batch)

    log(f"Inserted {inserted_count} SixHourTimeSeries rows.")
    return inserted_count


def main():
    """
    Main execution loop for the 6-hour snapshot ingestor.

    What: Load mapping, build lookup, then run a fetch/sleep loop forever.
    Why: Keeps the script running continuously on a 6-hour cadence.
