...
r"""
================================================================================
HOURLY VOLUME SNAPSHOT INGEST SCRIPT
================================================================================
What:
    - Polls the OSRS Wiki 1-hour price snapshot endpoint and inserts a single
      HourlyItemVolume row per item for each snapshot timestamp.
Why:
    - The alert system needs fast, local access to hourly volume data.
...
# What: Root URL used for the 1h snapshot endpoint.
# Why: Centralized for maintainability and reuse.
# How: Combine with "/1h" to build the request URL.
...
# REQUEST_TIMEOUT_SECONDS: HTTP timeout for the 1h snapshot request
...
# How: Used by chunked bulk_create loop below.
BULK_INSERT_BATCH_SIZE = 500

# SLEEP_INTERVAL_SECONDS: Delay between snapshot polls
# What: Sleep duration between fetch cycles.
# Why: The endpoint updates every 1 hour, so we poll on that cadence.
...
def fetch_1h_snapshot():
    """
    Fetch the latest 1-hour price snapshot from the OSRS Wiki API.

    What: GET /1h and parse the JSON response.
    Why: The 1h endpoint returns all item price data and a snapshot timestamp.
    ...
    """
    try:
        # response: Raw HTTP response from the 1h snapshot endpoint.
        ...
        log(f"ERROR: 1h API request failed: {exc}")
        return None

    if response.status_code != 200:
        log(f"ERROR: 1h API returned status {response.status_code}")
        return None
    ...
    if not isinstance(payload, dict):
        log("ERROR: 1h API payload is not a JSON object.")
        return None

    if "data" not in payload or "timestamp" not in payload:
        log("ERROR: 1h API payload missing required keys (data/timestamp).")
        return None
...
def snapshot_already_ingested(snapshot_timestamp):
    ...
    """
    return HourlyItemVolume.objects.filter(timestamp=snapshot_timestamp).exists()
...
def build_volume_objects(snapshot_data, lookup, snapshot_timestamp):
    """
    Build HourlyItemVolume objects from snapshot data.

    What: Converts API "data" dict into model instances with computed volume GP.
    ...
    How:
        ...
        - Treat missing numeric fields as 0 to satisfy non-null model fields.
        - Compute volume_gp = (avgHighPrice * highPriceVolume) + (avgLowPrice * lowPriceVolume).
    ...
    Returns:
        list: List of HourlyItemVolume objects ready for bulk_create.
    """
    ...
    for item_id_raw, item_payload in snapshot_data.items():
        ...
        # avg_high_price: Average instant-buy price (nullable in other models).
        # Why: Treat missing values as 0 so volume can still be computed.
        avg_high_price = item_payload.get("avgHighPrice")
        if avg_high_price is None:
            avg_high_price = 0

        # avg_low_price: Average instant-sell price (nullable in other models).
        # Why: Treat missing values as 0 so volume can still be computed.
        ...
        # volume_gp: Total GP traded during the snapshot interval.
        # Why: This is the core value stored in HourlyItemVolume.
        volume_gp = (avg_high_price * high_price_volume) + (avg_low_price * low_price_volume)

        # item_name: Human-readable name for the item (may be blank if unmapped).
        item_name = name_from_id(item_id, lookup) or ""

        objects_to_insert.append(HourlyItemVolume(
            item_id=item_id,
            item_name=item_name,
            volume=volume_gp,
...
def fetch_and_store_snapshot(lookup):
    """
    Fetch a single 1h snapshot and insert it into the database (if new).
    ...
    """
    # snapshot: Raw JSON payload for the current 1h snapshot.
    snapshot = fetch_1h_snapshot()
    if snapshot is None:
        return 0

    # snapshot_timestamp: Canonical snapshot timestamp as a string.
    # Why: HourlyItemVolume.timestamp is a CharField, and we need stable comparison.
    ...
    objects_to_insert = build_volume_objects(
...
def main():
    """
    Main execution loop for the hourly volume snapshot ingestor.

    What: Load mapping, build lookup, then run a fetch/sleep loop forever.
    Why: Keeps the script running continuously on a 1-hour cadence.
