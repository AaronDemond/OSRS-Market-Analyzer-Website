...
def getDataTimeSeries():
    ...
    bulk_create_committed_chunks(OneHourTimeSeries, all_objects, "OneHourTimeSeries")
...
def fetch_latest_one_hour_snapshot():
    ...
    # batch_size=500 stays within SQLite's variable limit (default max 999 per query).
    if new_records:
        bulk_create_committed_chunks(OneHourTimeSeries, new_records, "OneHourTimeSeries")
        print(f"\nAttempted insert of {len(new_records)} records into OneHourTimeSeries; duplicates were skipped.")
    else:
        print("\nNo records to insert.")
