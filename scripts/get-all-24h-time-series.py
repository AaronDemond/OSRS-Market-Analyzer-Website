...
def getDataTimeSeries():
    ...
    with transaction.atomic():
        TwentyFourHourTimeSeries.objects.bulk_create(
            all_objects,
            batch_size=BULK_INSERT_BATCH_SIZE,
            # ignore_conflicts=True,  # optional if you want to skip duplicates
        )
...
def fetch_latest_twenty_four_hour_snapshot():
    ...
    if new_records:
        with transaction.atomic():
            TwentyFourHourTimeSeries.objects.bulk_create(new_records, batch_size=BULK_INSERT_BATCH_SIZE)
        print(f"\nInserted {len(new_records)} records into TwentyFourHourTimeSeries.")
    else:
        print("\nNo records to insert.")
