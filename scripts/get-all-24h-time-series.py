...
from Website.models import HourlyItemVolume, FiveMinTimeSeries, TwentyFourHourTimeSeries
...
def make_twenty_four_hour_timeseries_objects(result_item, lookup):
    ...
    for x in result_item.get("data", []):
        ...
        objs.append(TwentyFourHourTimeSeries(
            item_id=item_id,
            item_name=item_name,
...
def getDataTimeSeries():
    ...
    all_objects = []
    for r in result:
        all_objects.extend(make_twenty_four_hour_timeseries_objects(r, lookup))

    print(f"prepared {len(all_objects)} rows, inserting...")

    bulk_create_committed_chunks(TwentyFourHourTimeSeries, all_objects, "TwentyFourHourTimeSeries")
...
def fetch_latest_twenty_four_hour_snapshot():
    """
    Fetch the most recent hourly volume datapoint for every item and insert it into the database.
    ...
    for result in raw_results:
        ...
        new_records.append(TwentyFourHourTimeSeries(
            item_id=item_id,
            item_name=item_name,
            avg_low_price=avg_low,
            avg_high_price=avg_high,
    ...
    # batch_size=500 stays within SQLite's variable limit (default max 999 per query).
    if new_records:
        bulk_create_committed_chunks(TwentyFourHourTimeSeries, new_records, "TwentyFourHourTimeSeries")
        print(f"\nAttempted insert of {len(new_records)} records into TwentyFourHourTimeSeries; duplicates were skipped.")
    else:
        print("\nNo records to insert.")
...
while True:
    ...
    print("Sleeping for twenty four hours and five minutes...")
    time.sleep((hours * 60 * 60) + (5*60))
    print("Fetching again...")
    fetch_latest_twenty_four_hour_snapshot()
